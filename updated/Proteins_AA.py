...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 12 14:55:28 2023
...
def ollivier_ricci_node(G, alpha):
    ...
    orc = OllivierRicci(G, alpha=0.5, verbose="INFO", weight = "weight")
...
def edge_sort(edge):
    ...
    edge_sort = edge[np.argsort(edge[:,0])]
...
def G_optimal_path_w(G1, G, path):
...
def G_optimal_path(G1, G, path):
    
    Gk = nk.nxadapter.nx2nk(G1)
...
def G_shortest_path_w(G1, G, path):
...
def path_pd_all_w(G1, G, path):
    ...
    for ii in range(len(path_new)):
        ...
        for jj in range(len(path_new[ii])):
            op_edge_, op_c0_, op_c1_ = G_optimal_path_w(G1, G, path_new[ii].iloc[jj, :])
            op_edge.append(op_edge_)
            op_c0.append(op_c0_)
            op_c1.append(op_c1_)
            op_c0_c1.append(np.abs(op_c0_ - op_c1_))

            sp_edge_, sp_c0_, sp_c1_ = G_shortest_path_w(G1, G, path_new[ii].iloc[jj, :])
...
def path_pd_all(G1, G, path):
    ...
    for ii in range(len(path_new)):
        ...
        for jj in range(len(path_new[ii])):
            op_edge_, op_c0_, op_c1_ = G_optimal_path(G1, G, path_new[ii].iloc[jj, :])
            op_edge.append(op_edge_)
            op_c0.append(op_c0_)
            op_c1.append(op_c1_)
            op_c0_c1.append(np.abs(op_c0_ - op_c1_))

            sp_edge_, sp_c0_, sp_c1_ = G_shortest_path(G1, G, path_new[ii].iloc[jj, :])
            sp_edge.append(sp_edge_)
            sp_c0.append(sp_c0_)
            sp_c1.append(sp_c1_)
            sp_c0_c1.append(np.abs(sp_c0_ - sp_c1_))
...
def edge_node_vec(xyz, G, orn, ore):
    ...
    for ii in range(len(e)):
        ...
        edge_vec_pd['ang'].iloc[ii] = 180*angle(np.round(pos[e[ii][0]], 3), np.round(pos[e[ii][1]],3), acute='True')/np.pi
...
#         print(cc)
    ...
    for ii in range(len(n)):
        ...
        node_vec_pd['x_ang'].iloc[ii] = 180*angle([1,0,0],np.round(pos[n[ii]]-center,3), acute='True')/np.pi
        node_vec_pd['y_ang'].iloc[ii] = 180*angle([0,1,0],np.round(pos[n[ii]]-center,3), acute='True')/np.pi
        node_vec_pd['z_ang'].iloc[ii] = 180*angle([0,0,1],np.round(pos[n[ii]]-center,3), acute='True')/np.pi
...
def edge_node_vec_torsion(xyz, G, edge_path):
    ...
    for ii in range(len(e)):
        ...
        edge_vec_pd['pos_1'].iloc[ii] = np.round(pos[e[ii][1]], 3)  #.astype('float16')
        edge_vec_pd['direction'].iloc[ii] = np.round(edge_vec_pd['pos_1'][ii] - edge_vec_pd['pos_0'][ii], 3)
        edge_vec_pd['ang'].iloc[ii] = 180 * angle(np.round(pos[e[ii][0]], 3), np.round(pos[e[ii][1]], 3),
                                                  acute='True') / np.pi
        edge_vec_pd['distance'].iloc[ii] = np.round(euclidean(pos[e[ii][1]], pos[e[ii][0]]), 3)
    ...
    center_node = 0
    center = xyz.loc['CA'][['x', 'y', 'z']].tolist()
...
def mirror_point(a, b, c, d, x1, y1, z1):  # mirrors a point across a plane defined by ax+by+cz+d=0
...
def find_pair_node_chiral(xyz, G, orn, ore):
    ...
    for ii in range(len(edge_list)):
        ...
        for jj in range(len(edge_list_d[ii])):
            if len(edge_list_d[ii][jj]) > 1 :
                ...
                if np.isclose(val0, val1, atol=0.2)==True and val0*val1 >0 :
                    edge_list_dv[ii].append(edge_list_d[ii][jj])

    non_zero_center = []
    for ii in range(len(center_node)):
        if edge_list_dv[center_node[ii]] != []:
            non_zero_center.append(center_node[ii])
    
    
    edge_list_dv_center = edge_list_dv[non_zero_center[0]]
    ...
    # drop = np.where(np.array(list(a.items()))[:, 0] == non_zero_center[0])[0][0]
    ...
    pair_new_ = [i[np.sort(np.unique(i, axis=0, return_index=True)[1])] for i in pair]
    pair_new = np.concatenate(pair_new_) 
...
def mirror_check_chiral(xyz, G, orn, ore, a, b, c, d):
    edge_vec, node_vec, center_node, center, pos = edge_node_vec(xyz, G, orn, ore)
    ...
    distances = cdist(np.asarray(m_point), np.vstack(node_vec['direction']))
    mirror_pair = np.vstack([np.arange(len(m_point)), np.argmin(distances, axis=0), np.min(distances, axis=0)]).T

    mirror_pair_ = mirror_pair[mirror_pair[:, 2].argsort()]

    mirror_pair__ = pd.DataFrame(mirror_pair_, columns=['node1', 'node2', 'dist'])
    mirror_pair__[['node1', 'node2']] = mirror_pair__[['node1', 'node2']].apply(sorted, axis=1, result_type='expand')
    mirror_pair__ = mirror_pair__[mirror_pair__['dist'] <= 0.1]  ###needs to be adjusted
    mirror_pair__.drop_duplicates(subset=['node2'], inplace=True)
    mirror_pair__.drop_duplicates(subset=['node1'], inplace=True)
    mirror_pair__.sort_values(by=['node1'], inplace=True)
    mirror_pair__.reset_index(drop=True, inplace=True)

    pair_ind_ = np.unique(
        np.append(mirror_pair__[['node1', 'node2']].to_numpy(), mirror_pair__[['node2', 'node1']].to_numpy(), axis=0),
        axis=0)

    pair_ind = []

    for ii in range(len(m_point)):
        if ii in pair_ind_[:, 0]:
            S = pair_ind_[np.where(pair_ind_[:, 0] == ii)[0]]
            pair_ind.append(np.squeeze(S)[1])
        else:
            pair_ind.append(np.nan)

    mirror_pair_unique = np.vstack([np.arange(len(m_point)), pair_ind]).T

    return mirror_pair_unique


def chirality_sign(xyz, G, edge_path, orn, ore, a, b, c, d):
    
    edge_vec, node_vec, center_node, center, pos = edge_node_vec_torsion(xyz, G, edge_path) #get torsion
    node_pair = mirror_check_chiral(xyz, G, orn, ore, a, b, c, d) #match nodes into pairs based on distances
...
#         print(n0)
    ...
    chirality = np.sum(np.asarray(plus_diff)*np.asarray(plus_val_diff))/len(plus_diff)-np.sum(np.asarray(minus_diff)*np.asarray(minus_val_diff))/len(minus_diff)

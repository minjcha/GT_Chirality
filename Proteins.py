...
#         print(ii)
        ...
        xyz0 = xyz.iloc[connect[ii][0],1:].to_numpy()
        xyz1 = xyz.iloc[connect[ii][1],1:].to_numpy()
    
        distance = euclidean(xyz0, xyz1)
        if distance <= cutoff*tol:
            edges.append(connect[ii])
    
    return edges

def no_weight(node, edge):
...
def norm_weight(xyz, node, edge):
    ...
    for ii in range(len(e)):
        weight.append(euclidean(xyz.iloc[e[ii][0],1:], xyz.iloc[e[ii][1],1:]))
    ...
    for ii in range(len(e)):
        G[e[ii][0]][e[ii][1]]['weight'] = norm_w[ii]
...
def node_connect_path(e_s, edge):
    
    a = np.where(np.isin(e_s, edge[0]))[0]
    b = np.where(np.isin(e_s, edge[1]))[0]
    
    a_u = np.unique(e_s[a].flatten())
    b_u = np.unique(e_s[b].flatten())
        
    a_u = a_u[a_u != edge[0]]
    a_u_u = a_u[a_u != edge[1]]
    
    b_u = b_u[b_u != edge[0]]
    b_u_u = b_u[b_u != edge[1]]
    
    common = np.intersect1d(a_u_u, b_u_u)
    connect = list(combinations(common, 2))
    
    path = pd.DataFrame(np.zeros((len(connect),4)), columns=['e0', 'e1', 'c0', 'c1'], dtype='int32')
    path['e0'] = edge[0]
    path['e1'] = edge[1]
    for ii in range(len(connect)):
        path['c0'].iloc[ii] = connect[ii][0]
        path['c1'].iloc[ii] = connect[ii][1]
...
def path_pd_all_w(G, path):
    ...
    for ii in range(len(path)):
        ...
        for jj in range(len(path[ii])):
            op_edge, op_c0, op_c1 = G_optimal_path_w(G, path[ii].iloc[jj,:])
            ...
            sp_edge, sp_c0, sp_c1 = G_shortest_path_w(G, path[ii].iloc[jj,:])
